...
Then, I realize that similar to others using memorization, my original impl overlooks one thing: we only care the status at the last index. Therefore, we only
need to track the status at last position. Inital staus at index i = -1 is true, then

Substring 0 to k is valid if:

1. 0 to k - 1 is valid and bits[k] is 0;
2. 0 to k - 1 is not valid (in this case, value at bits[k] is not important);

The above logic works, see isOneBitCharacter3, but implementation doesn't look as simple as isOneBitCharacter.
...
public:
	...
	bool isOneBitCharacter3(vector<int>& bits) {
		bool ans = true;
		int len = (int)bits.size() - 1;
		for (int i = 0; i < len; ++i)ans = (true == ans && 0 == bits[i]) || false == ans;
		return ans;
	}
};
